...
const PREDEFINED_TAGS = ['Veg', 'Non-Veg', 'Spicy', 'Chef Special', 'Bestseller', 'Gluten-Free', 'New'];
...
export default function AdminDashboard() {
  ...
  function toggleTag(tag: string) {
    let currentTags = itemTags ? itemTags.split(',').map(t => t.trim()).filter(Boolean) : [];
    if (currentTags.includes(tag)) {
      currentTags = currentTags.filter(t => t !== tag);
    } else {
      currentTags.push(tag);
    }
    setItemTags(currentTags.join(', '));
  }
                ...
                <div className="lg:col-span-1">
                  <div className="bg-white dark:bg-neutral-900 p-6 rounded-xl border border-gray-200 dark:border-neutral-800 shadow-sm md:sticky md:top-6 transition-colors duration-300">
                    ...
                    <form onSubmit={saveItem} className="space-y-4">
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-neutral-300 mb-2">Tags (Click to select)</label>
                        <div className="flex flex-wrap gap-2">
                          {PREDEFINED_TAGS.map(tag => {
                            const isActive = (itemTags || '').includes(tag);
                            let activeClass = 'bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 border-blue-300 dark:border-blue-700';
                            if (isActive) {
                              if (tag === 'Veg') activeClass = 'bg-green-100 dark:bg-green-900/50 text-green-700 dark:text-green-300 border-green-300 dark:border-green-700';
                              if (tag === 'Non-Veg') activeClass = 'bg-red-100 dark:bg-red-900/50 text-red-700 dark:text-red-300 border-red-300 dark:border-red-700';
                            }
                            ...
                            return (
                              <button
                                ...
                                className={`px-3 py-1.5 rounded-full text-[12px] font-bold border transition-all cursor-pointer ${
                                  isActive 
                                    ? activeClass 
                                    : 'bg-gray-50 dark:bg-neutral-900 text-gray-600 dark:text-neutral-400 border-gray-200 dark:border-neutral-700 hover:border-gray-400 dark:hover:border-neutral-500'
                                }`}
                              >
                                {tag.toLowerCase() === 'veg' && <span className={`inline-block w-2 h-2 rounded-full mr-1.5 ${isActive ? 'bg-green-500' : 'bg-gray-400'}`}></span>}
                                {tag.toLowerCase() === 'non-veg' && <span className={`inline-block w-2 h-2 rounded-full mr-1.5 ${isActive ? 'bg-red-500' : 'bg-gray-400'}`}></span>}
                                {tag}
                              </button>
                            );
                          })}
                    ...
                    <div className="mt-8 pt-8 border-t border-gray-200 dark:border-neutral-800">
                      <h3 className="text-sm font-bold text-gray-500 dark:text-neutral-400 uppercase tracking-wider mb-4 flex items-center gap-2">
                      ...
                      <div className="pointer-events-none">
                        <MenuItemCard 
                          item={{
                            ...
                            description: itemDesc || 'Your item description will appear here when you type it...',
                            price: parseFloat(itemPrice) || 0,
                            image_url: itemImage || (itemImageFile ? URL.createObjectURL(itemImageFile) : ''),
                            category_id: itemCat,
                            size: itemSize || null,
                            tags: itemTags ? itemTags.split(',').map(t => t.trim()).filter(Boolean) : null,
                            created_at: ''
